...
class TMY(object):
    ...
    geojson_url = ('https://github.com/NREL/EnergyPlus/'
                   'raw/develop/weather/master.geojson')

    download_dir_path = Path(WEATHER_DIR)

    def __init__(self, country: str, temperature_file: str,
        ...
        self.lookup_str = self.make_lookup_str(
            country=self.country, temperature_file=self.temperature_file,
            state=self.state)
    ...
    def download_data(cls, lookup_str: str) -> Path:
        ...
        data = r.json()
        print(lookup_str)
        matches = [m for x in data['features']
    ...
    @staticmethod
    def make_lookup_str(country: str, temperature_file: str,
