...
def clean_df(df):
	...
	date_range = pd.date_range(str(df.index[0].year)+'-01-01 00:00:00', str(df.index[0].year)+'-12-31 23:00:00', freq='1H')
	...
	df = df.sort_index() # sort to make new rows in place, otherwise the Nan rows are at the end
	df = df.interpolate() # interpolate values
	df = df.fillna(method='ffill') # fill with rest NaN with value of previous row
...
if __name__ == '__main__':
	...
	epw = EPW()
	epw_file = os.path.join(EPW_DIR, 'USA_NY_New.York-J.F.Kennedy.Intl.AP.744860_TMY3.epw')
	epw.read(epw_file)
	...
	for i,wd in enumerate(epw.weatherdata):
		...
		if value_temp >= 70:
			value_temp = 69 # condition of EPW package, value need to be smaller 70.0 for field dry_bulb_temperature
		if value_temp <= -70:
			value_temp = -69
		...
		if value_pressure >= 120000:
			value_pressure = 119999 # condition of EPW package, value need to be smaller 120000 for field atmosphere pressure 
