...
EPHISTORY_PATH = os.path.join(SUPPORT_DIR, 'ep_weather_stations.xlsx')
...
st.markdown(
        "### Select CDD and HDD threshold"
    )

col1, col2 = st.columns(2)

with col1:
    hdd = st.number_input('HDD Threshold',
                        min_value=30,
                        max_value=100,
                        value=70,
                        step=1,
                        on_change=set_to_false)
with col2:
    cdd = st.number_input('CDD Threshold',
                        min_value=30,
                        max_value=100,
                        value=80,
                        step=1,
...
if "downloaded" in st.session_state.keys() and st.session_state["downloaded"]:
    ...
    st.download_button(
        ...
        data=convert_df(df),
        file_name=(st.session_state["station"].o.op_file_name +
        '-' + freq.lower() + '.' + type_of_output.lower())
        )
